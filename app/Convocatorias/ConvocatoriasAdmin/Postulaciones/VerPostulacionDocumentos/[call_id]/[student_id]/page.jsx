...
import {
  apiCallRegion,
  apitypePos,
} from "@/app/api/ConvocatoriasEstudiante/typePostulationConv";
...
function VerPostulacionDocumentos({ params }) {
  ...
  if (!session) {
    ...
    if (applicationData.documents?.eps_certificate) {
      return (
        <div className="p-8">
          ...
          <p className="text-2xl text-justify pl-8 pr-10">
            A continuacion se encuentran los documentos enviados por el
            estudiante para postularse a esta convocatoria.
          </p>
          <br />
          <p className="text-2xl text-justify pl-8 pr-10">
    ...
    } else if (applicationData.documents?.passport) {
      return (
        <div className="p-8">
          <h1 className="text-black font-bold text-[35px]">
            Documentos de la Postulación
